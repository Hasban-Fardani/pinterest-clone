                ...
                <div class="mb-4">
                    <form action="{{ route('posts.index') }}" method="get" class="flex w-1/2 gap-2">
                        <input type="text" name="q" class="grow input input-bordered focus:outline-none"
                            placeholder="Search" value="{{ request('q') }}" onfocus="this.value = this.value;"
                            autofocus />
                    ...
                    @foreach ($posts as $post)
                        <div class="w-full mb-4 break-inside-avoid relative group">
                            <div class="relative max-h-fit">
                                <div
                                    class="-z-10 group-hover:z-10 absolute backdrop-blur-md bg-white/20 rounded-lg w-full h-full">
                                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex gap-2">
                                        <button class="btn btn-warning" aria-haspopup="dialog" aria-expanded="false"
                                            aria-controls="modal-edit-post"
                                            data-overlay="#modal-edit-post" onclick="editPost({{ $post->id }}, '{{ $post->title }}', '{{ $post->image }}')">Edit</button>
                                        <button class="btn btn-error"
                                            onclick="confirmDelete({{ $post->id }}, '{{ $post->title }}')">Hapus</button>
                                        <form action="{{ route('posts.destroy', $post) }}" method="post"
                                            id="delete-form-{{ $post->id }}">@csrf @method('delete')</form>
                                    </div>
                                </div>
                                <img src="{{ '/storage/posts/' . $post->image }}" alt="{{ $post->title }}"
                            ...
                            <div class="flex items-center gap-3">
                                <div class="avatar">
                                    <div class="size-10 rounded-full">
                                        <img src="{{ $post->user->avatar ?? 'https://ui-avatars.com/api/?name=user' }}"
                                            alt="{{ $post->user->name }}">
                                    </div>
                                </div>
                                <p>{{ $post->user->name }}</p>
    ...
    <x-slot name="js">
        <script>
            function editPost(id, title, image) {
                ...
                document.getElementById('image-edit-post-preview').src = '/storage/posts/' + image;
